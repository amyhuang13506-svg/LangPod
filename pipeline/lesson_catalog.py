...
SCENE_TEMPLATES = [
    ...
    {
        "slug": "dentist", "category": "health", "icon": "mouth.fill",
        "zones": [
            {"id": "reception_history", **_t("前台与病史", "Reception & History"),
             "hint": "dental reception desk, clipboard medical history form, insurance card, waiting room chairs, tooth model on counter"},
            {"id": "in_the_chair", **_t("坐上牙椅检查", "In the Dental Chair"),
             "hint": "dental chair, bright overhead lamp, mouth mirror, dental probe, bib/napkin clip, X-ray film of teeth, spit sink"},
            {"id": "cleaning_and_advice", **_t("洁牙与医嘱", "Cleaning & Advice"),
             "hint": "scaler tool, toothbrush, dental floss, fluoride tray, tube of toothpaste, model showing brushing, next-appointment card"},
        ],
        "countries": {
            "us": {"anchor": "dental insurance, copay, cleaning every 6 months, cavity filling", "title": _t("在美国看牙医", "At the Dentist in the US")},
            "uk": {"anchor": "hard to find an NHS dentist, private practice, band 1/2/3 charges", "title": _t("在英国看牙医", "At the Dentist in the UK")},
            "au": {"anchor": "dental not covered by Medicare, private cost, health fund extras", "title": _t("在澳洲看牙医", "At the Dentist in Australia")},
            "ca": {"anchor": "dental not on the health card, workplace benefits plan", "title": _t("在加拿大看牙医", "At the Dentist in Canada")},
            "nz": {"anchor": "free dental under 18, pricey private dentist for adults", "title": _t("在新西兰看牙医", "At the Dentist in NZ")},
            "sg": {"anchor": "polyclinic dental vs private clinic, CHAS subsidy card", "title": _t("在新加坡看牙医", "At the Dentist in Singapore")},
        },
    },
    {
...
# 每日新增课堂的场景库：每天由 generate_daily_lesson.py 按国家轮流（us→uk→au→ca→nz→sg）
# 取下一个未生成的场景，标 is_daily=true。不进 all_lessons() 的 125 正式课。
# 每国 10 个，够轮两个月；用尽后再补或由 GPT 续写。所有 slug 与 125 正式课不重复。
DAILY_SCENE_BACKLOG = [
    # ---------- 🇺🇸 US ----------
    {"country": "us", "slug": "ikea_run", "title": _t("在宜家买家具", "The IKEA Run"), "category": "settling",
     "hint": "showroom room-sets, flat-pack warehouse aisle/bin numbers, yellow trolley, checkout, meatballs in the cafe, allen key"},
    {"country": "us", "slug": "black_friday", "title": _t("黑五扫货", "Black Friday Shopping"), "category": "food",
     "hint": "doorbuster deals signs, line outside before opening, shopping cart, price tags slashed, gift receipt, store map"},
    {"country": "us", "slug": "car_towed", "title": _t("车被拖走了怎么办", "My Car Got Towed"), "category": "settling",
     "hint": "no parking sign fine print, empty parking spot, tow truck, impound lot fence, release fee counter, printed ticket"},
    {"country": "us", "slug": "tipping_everything", "title": _t("到处都要小费？", "Tipping Culture Decoded"), "category": "food",
     "hint": "payment tablet with tip screen, 18/20/25% buttons, tip jar on counter, receipt with tip line, delivery bag"},
    {"country": "us", "slug": "jury_duty", "title": _t("收到陪审团传票", "Jury Duty Summons"), "category": "settling",
     "hint": "summons letter in the mail, courthouse metal detector, waiting room with numbered badge, juror ID card, water cooler"},
    {"country": "us", "slug": "farmers_market", "title": _t("逛周末农夫市集", "The Farmers Market"), "category": "food",
     "hint": "canvas stall tents, crates of vegetables, honey jars, tote bag, cash and card reader, free samples on toothpicks"},
    {"country": "us", "slug": "food_truck", "title": _t("路边餐车点餐", "Ordering at a Food Truck"), "category": "food",
     "hint": "food truck window, chalkboard menu, order number ticket, condiment station, napkins, picnic table nearby"},
    {"country": "us", "slug": "venmo_split", "title": _t("Venmo 分账", "Splitting the Bill on Venmo"), "category": "settling",
     "hint": "phone with payment app, split the check, request money, QR code to scan, emoji note, dinner receipt on table"},
    {"country": "us", "slug": "super_bowl_party", "title": _t("超级碗观赛聚会", "A Super Bowl Party"), "category": "social",
     "hint": "big TV with the game, couch, chips and dip bowls, chicken wings platter, team jersey, cheering, commercials"},
    {"country": "us", "slug": "post_office_run", "title": _t("去邮局寄包裹", "A Trip to the Post Office"), "category": "settling",
     "hint": "USPS counter, package on scale, shipping label, flat-rate box, stamps sheet, tracking receipt, PO box wall"},
    # ---------- 🇬🇧 UK ----------
    {"country": "uk", "slug": "sunday_roast", "title": _t("周日烤肉聚餐", "Sunday Roast"), "category": "food",
     "hint": "roast beef plate, yorkshire pudding, gravy boat, roast potatoes, carvery counter, pint next to the plate"},
    {"country": "uk", "slug": "boxing_day", "title": _t("Boxing Day 抢折扣", "Boxing Day Sales"), "category": "food",
     "hint": "post-Christmas sale signs, returns queue, gift receipt, shopping bags, discount rail of clothes"},
    {"country": "uk", "slug": "chippy", "title": _t("炸鱼薯条店打包", "At the Chippy"), "category": "food",
     "hint": "fish and chips wrapped in paper, mushy peas tub, salt and vinegar, chip shop counter menu, wooden fork, curry sauce"},
    {"country": "uk", "slug": "afternoon_tea", "title": _t("英式下午茶", "Afternoon Tea"), "category": "food",
     "hint": "three-tier cake stand, scones with clotted cream and jam, teapot and china cups, finger sandwiches, strainer"},
    {"country": "uk", "slug": "car_boot_sale", "title": _t("逛后备箱二手集市", "The Car Boot Sale"), "category": "food",
     "hint": "open car boots in a field, folding tables of bric-a-brac, second-hand toys, haggling over coins, carrier bag"},
    {"country": "uk", "slug": "footy_pub", "title": _t("在酒吧看球赛", "Watching the Footy at the Pub"), "category": "social",
     "hint": "big screen with the match, pint of lager, team scarf, crowd cheering, bar with taps, packet of crisps"},
    {"country": "uk", "slug": "bin_day", "title": _t("垃圾分类回收日", "Bin Day"), "category": "settling",
     "hint": "wheelie bins at the kerb, colour-coded lids, recycling box, collection calendar on the fridge, bin lorry"},
    {"country": "uk", "slug": "greggs_run", "title": _t("Greggs 买香肠卷", "A Greggs Run"), "category": "food",
     "hint": "bakery display case, sausage roll, steak bake, meal deal sign, paper bag, coffee, queue at the till"},
    {"country": "uk", "slug": "bonfire_night", "title": _t("篝火之夜看烟花", "Bonfire Night"), "category": "social",
     "hint": "big bonfire, fireworks in the sky, sparklers in hand, toffee apple, hot dog, woolly hat and scarf, crowd"},
    {"country": "uk", "slug": "corner_shop", "title": _t("街角便利店买东西", "At the Corner Shop"), "category": "food",
     "hint": "cramped shop shelves, newspaper rack, lottery counter, meal deal fridge, top-up for phone, chocolate bars by the till"},
    # ---------- 🇦🇺 AU ----------
    {"country": "au", "slug": "medicare_signup", "title": _t("办澳洲 Medicare", "Signing Up for Medicare"), "category": "health",
     "hint": "Medicare office counter, enrolment form, green Medicare card, proof of residency documents, queue ticket machine"},
    {"country": "au", "slug": "magpie_season", "title": _t("躲避喜鹊俯冲的季节", "Magpie Swooping Season"), "category": "social",
     "hint": "swooping magpie, bike helmet with zip ties sticking up, warning sign on a pole, cyclist ducking, spring park"},
    {"country": "au", "slug": "servo_stop", "title": _t("在加油站便利店", "A Stop at the Servo"), "category": "settling",
     "hint": "petrol pump, fuel nozzle, pie warmer inside, drink fridge, pay at the console, meat pie and sauce, receipt"},
    {"country": "au", "slug": "bunnings_snag", "title": _t("Bunnings 门口吃香肠", "The Bunnings Sausage Sizzle"), "category": "social",
     "hint": "sausage sizzle stand, snag on white bread, onions and tomato sauce, gold coin donation, hardware store entrance"},
    {"country": "au", "slug": "surf_flags", "title": _t("海滩红黄旗之间游泳", "Swim Between the Flags"), "category": "social",
     "hint": "red and yellow surf flags, lifeguard tower, rip current sign, boardshorts, zinc on the nose, esky on the sand"},
    {"country": "au", "slug": "bottle_o", "title": _t("在酒类专卖店买酒", "At the Bottle-O"), "category": "food",
     "hint": "drive-through bottle shop, six-pack of tinnies, wine shelf, cold room fridge, ID check for age, esky ice bag"},
    {"country": "au", "slug": "tradie_talk", "title": _t("听懂工友俚语", "Talking Like a Tradie"), "category": "social",
     "hint": "hi-vis vest, ute with tools, hard hat, smoko break with a coffee, esky, worksite fence, thermos"},
    {"country": "au", "slug": "footy_afl", "title": _t("看 AFL 澳式橄榄球", "Going to the Footy (AFL)"), "category": "social",
     "hint": "oval stadium, team scarf, meat pie and chips, goal posts, big screen scoreboard, crowd in team colours"},
    {"country": "au", "slug": "centrelink", "title": _t("去 Centrelink 办事", "Sorting Things at Centrelink"), "category": "settling",
     "hint": "service centre counter, queue ticket, forms clipboard, ID documents, self-service kiosk, waiting chairs"},
    {"country": "au", "slug": "woolies_run", "title": _t("在 Woolworths 采购", "A Woolies Shop"), "category": "food",
     "hint": "green shopping trolley, self-checkout, reusable bags, Rewards card scan, fresh produce aisle, weekly specials tag"},
    # ---------- 🇨🇦 CA ----------
    {"country": "ca", "slug": "double_double", "title": _t("Tim Hortons 点单黑话", "Speaking Tims"), "category": "food",
     "hint": "Tim Hortons counter, double double coffee, box of Timbits, drive-thru speaker, roll up the rim cup, donut display"},
    {"country": "ca", "slug": "poutine_night", "title": _t("吃一份 Poutine", "A Plate of Poutine"), "category": "food",
     "hint": "fries with cheese curds and gravy, cardboard tray, fork, diner counter, napkins, pop can on the side"},
    {"country": "ca", "slug": "winter_tires", "title": _t("换冬季胎", "Swapping to Winter Tires"), "category": "settling",
     "hint": "garage service bay, stack of winter tires, mechanic with wrench, tire tread close-up, appointment desk, snow outside"},
    {"country": "ca", "slug": "sugar_shack", "title": _t("枫糖季去 Sugar Shack", "A Visit to the Sugar Shack"), "category": "food",
     "hint": "maple syrup on snow (tire d'érable), stick to roll it, cabin in the woods, sap buckets on trees, pancakes, wood stove"},
    {"country": "ca", "slug": "canada_post", "title": _t("去 Canada Post 取包裹", "Picking Up a Parcel at Canada Post"), "category": "settling",
     "hint": "postal counter inside a pharmacy, parcel pickup notice card, ID check, package on the counter, mailbox key, stamps"},
    {"country": "ca", "slug": "blue_bin_day", "title": _t("垃圾与回收分类日", "Garbage & Recycling Day"), "category": "settling",
     "hint": "blue recycling bin, green compost bin, black garbage bin at the kerb, collection schedule magnet, raccoon-proof lid"},
    {"country": "ca", "slug": "cottage_weekend", "title": _t("去 Cottage 过长周末", "A Long Weekend at the Cottage"), "category": "social",
     "hint": "lakeside cabin, canoe on the dock, campfire and marshmallows, cooler of drinks, bug spray, muskoka chair, loon on the lake"},
    {"country": "ca", "slug": "curling_night", "title": _t("周末打冰壶", "Curling Night"), "category": "social",
     "hint": "curling sheet ice, granite stone with handle, broom sweeping, house target rings, sliders on shoes, scoreboard"},
    {"country": "ca", "slug": "service_ontario", "title": _t("办健康卡与驾照", "Renewing Your Health Card"), "category": "settling",
     "hint": "ServiceOntario counter, health card photo, driver's licence renewal, number ticket, proof of address, camera for photo"},
    {"country": "ca", "slug": "hockey_arena", "title": _t("带娃去看冰球训练", "Kids' Hockey at the Arena"), "category": "social",
     "hint": "cold ice rink, kid in hockey gear and helmet, hockey stick and puck, bench, arena seats, hot chocolate, gear bag"},
    # ---------- 🇳🇿 NZ ----------
    {"country": "nz", "slug": "bach_weekend", "title": _t("去朋友的度假屋过周末", "A Weekend at the Bach"), "category": "social",
     "hint": "simple beach house (bach), jandals at the door, beach cricket set, fish and chips wrapped in paper, chilly bin, deck chairs"},
    {"country": "nz", "slug": "pak_n_save", "title": _t("在 Pak'nSave 省钱采购", "Shopping at Pak'nSave"), "category": "food",
     "hint": "bright yellow warehouse aisles, budget trolley, pack-your-own bags at checkout, price-fighter tags, receipt, reusable bags"},
    {"country": "nz", "slug": "tramping_hut", "title": _t("山野徒步住 DOC 小屋", "Tramping to a DOC Hut"), "category": "social",
     "hint": "backpack and boots, orange track marker pole, wooden DOC hut, bunk beds inside, hut pass ticket, drink bottle, muddy trail"},
    {"country": "nz", "slug": "wof_check", "title": _t("给车做 WOF 车检", "Getting a WOF"), "category": "settling",
     "hint": "garage inspection bay, mechanic checking lights and tyres, WOF sticker on windscreen, checklist clipboard, service counter"},
    {"country": "nz", "slug": "countdown_shop", "title": _t("在超市买日用品", "A Supermarket Run"), "category": "food",
     "hint": "supermarket trolley, Onecard loyalty scan, self-checkout, fresh produce, reusable bags, weekly specials shelf tag"},
    {"country": "nz", "slug": "rugby_arvo", "title": _t("看 All Blacks 橄榄球", "Watching the All Blacks"), "category": "social",
     "hint": "TV with rugby match, black team jersey, snacks on the coffee table, haka on screen, scoreboard, mates on the couch"},
    {"country": "nz", "slug": "island_ferry", "title": _t("坐渡轮过海", "Taking the Ferry"), "category": "settling",
     "hint": "harbour ferry terminal, gangway, AT HOP card tag-on, seats with a sea view, life jackets sign, wharf and boats"},
    {"country": "nz", "slug": "kiwi_christmas", "title": _t("夏天过圣诞 BBQ", "A Summer Christmas BBQ"), "category": "social",
     "hint": "backyard barbecue in summer, pavlova with kiwifruit, jandals and shorts, Santa hat, chilly bin of drinks, cricket on the lawn"},
    {"country": "nz", "slug": "trademe_car", "title": _t("在 Trade Me 买二手车", "Buying a Used Car on Trade Me"), "category": "settling",
     "hint": "second-hand car for sale, odometer close-up, test drive keys, listing on a phone, handshake, WOF and rego papers"},
    {"country": "nz", "slug": "gumboots_farm", "title": _t("去乡下农场做客", "A Day on the Farm"), "category": "social",
     "hint": "gumboots at the door, sheep in a paddock, farm gate, quad bike, woolly jumper, sheepdog, muddy track"},
    # ---------- 🇸🇬 SG ----------
    {"country": "sg", "slug": "ns_talk", "title": _t("听懂新加坡人聊 NS", "When Friends Talk About NS"), "category": "social",
     "hint": "green army field pack, book-out at the gate, camo uniform drying on a rack, reservist letter, canteen, mates chatting"},
    {"country": "sg", "slug": "gss_shopping", "title": _t("大促季扫货", "Great Singapore Sale"), "category": "food",
     "hint": "Orchard Road mall sale banners, shopping bags, discount racks, GST refund counter for tourists, queue, price tags"},
    {"country": "sg", "slug": "chope_seat", "title": _t("用纸巾占座", "Choping a Seat"), "category": "food",
     "hint": "packet of tissues placed on a hawker table to reserve it, empty chairs, food court crowd, tray of food, table number"},
    {"country": "sg", "slug": "cai_png", "title": _t("点一份经济杂菜饭", "Ordering Cai Png (Economy Rice)"), "category": "food",
     "hint": "steam tray of mixed dishes, tongs and ladle, plate of rice, pointing at dishes, uncle behind the counter, price guess"},
    {"country": "sg", "slug": "durian_buy", "title": _t("买榴莲", "Buying Durian"), "category": "food",
     "hint": "spiky durians in a pile, stall uncle opening one with a glove, plastic tray of fleshy pods, weighing scale, mao shan wang sign"},
    {"country": "sg", "slug": "grabfood_order", "title": _t("叫 GrabFood 外卖", "Ordering GrabFood"), "category": "food",
     "hint": "phone with delivery app, green delivery bag, rider on a motorbike, order tracking map, unit number and lift lobby, receipt"},
    {"country": "sg", "slug": "cny_visiting", "title": _t("农历新年拜年", "Chinese New Year Visiting"), "category": "social",
     "hint": "red ang bao packets, mandarin oranges in pairs, pineapple tarts, bak kwa, reunion table, festive decorations, greetings"},
    {"country": "sg", "slug": "bto_flat", "title": _t("申请 BTO 组屋", "Applying for a BTO Flat"), "category": "settling",
     "hint": "HDB show flat, floor plan on the wall, application form on a tablet, queue number, keys, block model, balloting notice"},
    {"country": "sg", "slug": "carpark_coupon", "title": _t("停车买 coupon", "Parking with Coupons"), "category": "settling",
     "hint": "paper parking coupon, tearing tabs for date and time, coupon on the dashboard, open-air carpark lot, HDB carpark sign"},
    {"country": "sg", "slug": "void_deck_do", "title": _t("组屋楼下的红事", "A Void Deck Celebration"), "category": "social",
     "hint": "void deck under an HDB block, round tables and red chairs, catered buffet, festive banner, folding tables, neighbours gathering"},
]
