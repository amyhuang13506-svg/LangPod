...
/// 「探索」页：硅谷大佬演讲 + 公司发布会以外的优质英文视频内容。
/// - TED / Stanford / YC 经典演讲
/// - Veritasium / Kurzgesagt / 3Blue1Brown 科普
/// - Huberman Lab / Diary of a CEO / Modern Wisdom 思想类访谈
/// - MKBHD 等评测
struct ExploreView: View {
    ...
    private var explorePodcasts: [RawPodcast] {
        podcasts.filter { ($0.category ?? "") == "explore" }
    }

    private var topics: [String] {
        var set = Set<String>()
        for p in explorePodcasts {
                    ...
                    VStack(alignment: .leading, spacing: 18) {
                        ...
                        if filtered.isEmpty {
                            ...
                            ForEach(filtered) { p in
                                feedRow(p).onTapGesture { selectedPodcast = p }
                            }
                        }
                    }
                    .padding(.bottom, 60)
    ...
    private var intro: some View {
        VStack(alignment: .leading, spacing: 6) {
            Text("超出硅谷的英文世界")
                ...
                .foregroundStyle(Color.textPrimary)
                .tracking(-0.3)
            Text("TED / Huberman / Veritasium 等热门英文 YouTube 内容")
                .font(.system(size: 13))
    ...
    private var emptyState: some View {
        VStack(spacing: 8) {
            ...
            Text("探索内容生成中")
                ...
                .foregroundStyle(Color.textSecondary)
            Text("Pipeline 正在扫描 TED / Huberman / Veritasium 等频道，明日开始上线")
                .font(.system(size: 12))
                .foregroundStyle(Color.textTertiary)
