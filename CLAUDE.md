...
### 2026-08-03 — 数据诊断 + Day 0 改造 + 推送修复 + 成本止血（CDN + 音视频分离）

**数据诊断（友盟漏斗 8/2 首日数据）：**
- 友盟自定义事件 7/31 才注册（之前上报全被丢弃，不可追溯）；42 个事件已批量注册
- 漏斗结论：新用户首日仅 10.6% 完成第一集；播放漏斗 play_start 93 → round1 70（75% 健康）→ 全 5 遍完成 31（44% 断崖）——**用户死在重复遍数里，不是开头**
- push_opened 全天 1 人 → 排查发现**早间推送 7/26 起停摆 9 天**：IPRoyal 代理流量耗尽（402）→ YouTube 管线 0 新增 → 推送队列空。副作用：首页原声内容 9 天没更新，DAU 从 451 跌到 323
- 下午 5 点集数推送从未存在（crontab 只有注释；generate_daily 没有 enqueue 调用）
- 推送授权率其实健康（token/新增 ≈ 60-72%，1857 台可推设备）

**Day 0 体验改造（方案：docs/day0_体验与推送修复方案_20260803.md）：**
- **第 1 遍完成即完播**：完成页/生词卡/streak/任务打勾全部提前到第 1 遍结束；新增「再听加深」CTA 继续 2-5 遍；episode_abandon 语义收窄为"第 1 遍没听完"
- **三连弹拆除**：落地只弹今日计划；推送授权改为首集完成页铺垫卡（拒绝不烧系统机会，streak≥3 再问）；ATT 延到第二次冷启动（首启放行 Adjust 首包走 SKAdNetwork，投放信号不丢）
- 新埋点 push_permission_result（待友盟注册）；本地推送仲裁改为实时查权限

**推送系统修复（服务器）：**
- generate_daily 接 enqueue_episode + crontab 加 17:00 flush --type episode（每级去重发一条）
- check_push_health.py 看门狗（每早 8 点）：队列连续 2 次空转 → APNs 告警推到开发机 sandbox 设备（已实测送达）
- IPRoyal 充值后管线恢复（注意：流量到账有延迟，测通 ipify ≠ 立即可用）

**成本止血（7 月 OSS 账单 ¥1034，传输加速占 ¥671）：**
- 根因：原声播客存的是完整视频 mp4（321 个共 38GB，平均 118MB），App"听"的场景在为视频字节付流量，且无缓存
- **音视频分离**：pipeline 自动抽 m4a（acodec copy）+ backfill_raw_audio.py 存量 312 条全部回填成功（内网 endpoint 零流量费）；App 默认播 m4a（体积 1/4），封面加「看视频」按钮按需加载 mp4，进度无缝衔接
- **CDN 上线**：castlingo-cdn.myblackhole.pro（阿里云 CDN 海外版，免备案，复用 BlackHole 域名）→ 回源 OSS；Range 回源已开；Let's Encrypt 证书 acme.sh 自动续期（renew_cdn_cert.sh 钩子自动重传）；实测 HTTPS/206/边缘缓存命中全通过
- App OSSBaseURL 切到 CDN 域名；预期流量成本降到 ~1/10
- ⚠️ 传输加速暂不能关（老版本用户还在走 accelerate 域名，等新版覆盖后再关）

**发现待处理：**
- 友盟后台弹"套餐剩余 0 天"续费提醒（只影响统计不影响推送——推送全走苹果 APNs）
- 付费墙压力过早：124 人看到 vs 31 人完播；attempt 17 → success 4（多为主动取消）
- 词义配对（3 人/天）、连词成句（1 人/天）基本是死功能

### 明日待办（2026-08-04）

**优先级 1：**
- [ ] 友盟「未注册事件」列表注册 push_permission_result
- [ ] 早 7:30 验证推送恢复（9 天来第一条）+ 看门狗 8 点应报健康
- [ ] 打包提审新版本（Day 0 改造 + 音频优先 + CDN 一起发）

**优先级 2：**
- [ ] OSS 访问日志落地后核对真实流量构成
- [ ] 友盟套餐续费弹窗确认（什么套餐、影响什么）
- [ ] 观察 first_episode_complete 是否从 10.6% 抬升（发版后）

### 2026-08-17 ~ 08-21 — 留存攻坚：结算时刻 + 续看推送 + 听力测试体系（多 session 汇总）

**背景**：D1 ~18-24%、D7 ~5%。诊断：激活修好后卡在"回访动机"。三板斧依次落地：

**① 原声结算时刻**（docs/原声结算时刻方案_20260817.md）：
- raw 视频退出（≥180s）弹结算卡：真实听力 N 分钟 + 本次生词 + 3 道主旨理解题（GPT 生成，6 语解析）
- quiz.json 全量回填：raw 358 条 + episodes ~880 集；generate_daily 已串入出题步骤
- 8/20 数据：测题打开率 52%、完成率 94% —— 验证通过

**② 续看推送**（docs/续看推送方案_20260818.md）：看一半的视频《X》还剩 N 分钟 → 新仲裁层级 + raw:{id} 深链

**③ 听力测试体系**（docs/听力测试体系方案_20260821.md）：
- 首页三段：首页 / 学习 / 听测（服务器 app_config.json default_home_segment 可零发版切默认页）
- 测验流：盲听整集（真实时长，无 90s 窗口）→ 答 3 题 → 看原文；连测手动点下一集（不自动播）
- 等级爬坡：连续 3 套全对升级 / 3 套 ≤1 题降级（24h 内重测同集不计）；蓝卡明示规则
- 听力库：三级合并可重听语料，今日免费 / 往期 Pro；蓝卡测完轮换随机未测集（锁定集 🔒→付费墙）
- 新用户首弹分级测试：从 onboarding 自选级别起测，全对建议升一级 / ≤1 建议降一级；失败自动承接今日计划弹窗
- 每日任务第一格固定 = 今日听力测试
- 等级圆点：灰银 #B9C2CE / 浅绿 #7FDD9B / 绿 #1FA750（SwiftUI 画圆，非 emoji）
- 统计独立 testDays 日志（重测旧集不改写周日历/连续天数）

**8/21 收尾审查修复（两路 agent 交叉审查确认 11 项、修 9 项）**：
- 付费墙旁路（蓝卡轮换往期集免费可进）/ 双音频同播 / 重测改写历史统计 / 迟到响应窜集 /
  升降级后连测断流 / 新用户开始测试网络失败死胡同 / 弹窗叠加 / 短集不自动进答题 / 埋点双发
- 埋点 4 个已注册：listening_test_start/complete、onboarding_test_skip、home_segment_switch

**已知遗留**：
- 冷启动推送任务深链极窄竞态（raw 路径有 pending 兜底、任务路径没有）
